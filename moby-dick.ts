// moby-dick.ts
// A simple script to print the first N words of Moby Dick Chapter 1: "Loomings"

const MOBY_DICK_CHAPTER_1 = `Call me Ishmael. Some years ago--never mind how long precisely--having
...
as if striving to get a still better seaward peep. But these are all
...
function main(): void {
  ...
  if (isNaN(n) || n <= 0) {
    console.error("Usage: npx ts-node moby-dick.ts <N>");
    console.error("  N must be a positive integer");
    process.exit(1);
  }
  
  const words = MOBY_DICK_CHAPTER_1.split(/\s+/);
  const firstNWords = words.slice(0, n);
  
  console.log(firstNWords.join(" "));
}
